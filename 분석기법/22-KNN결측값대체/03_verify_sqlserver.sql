...
SELECT TOP 8 donor_customer_id, donor_target_value, ROUND(dist_normalized, 4) AS dist
FROM pairwise_distance_final
WHERE target_customer_id = 'C018'
ORDER BY dist_normalized ASC, donor_customer_id ASC;

-- 7) 문자열 컬럼: NULL / '' / ' ' 집계 — SQL Server는 null_count와
--    empty_string_count가 서로 다른 행(C003 vs C005)을 가리킨다.
--    Oracle 버전(03_verify_oracle.sql)의 결과와 반드시 대조할 것.
SELECT
...
-- 7-1) LEN vs DATALENGTH 차이 재확인 — C006(공백 문자열 ' ')에서
--      LEN은 0(뒤쪽 공백 잘림), DATALENGTH는 1(실제 저장 바이트)이어야 한다.
SELECT customer_id, coupon_code_raw, coupon_len_trimmed, coupon_len_actual_bytes
FROM null_identification
...
SELECT r.customer_id, r.frequency, r.monetary, r.target_missing_value,
       CASE WHEN r.target_missing_value IS NULL THEN N'결측' ELSE N'정상값(0 포함)' END AS status
...
UNION ALL
SELECT
    N'after(20행, knn_imputed_strict로 채움)',
    ROUND(AVG(ISNULL(original_value, knn_imputed_strict)), 2),
    ROUND(MIN(ISNULL(original_value, knn_imputed_strict)), 2),
    ROUND(MAX(ISNULL(original_value, knn_imputed_strict)), 2)
FROM impute_practice_result;
