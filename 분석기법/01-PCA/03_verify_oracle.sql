...
-- 실제로 Oracle에 연결하려면 02_analyze.py의 dbutil.get_conn()을 cx_Oracle/oracledb
-- 커넥션으로 교체하고 pandas.to_sql(..., dtype=...)로 아래와 같은 스키마에 맞춰 적재한다.
...
CREATE TABLE pca_scores (
    ...
    PC1 NUMBER, PC2 NUMBER, PC3 NUMBER, PC4 NUMBER, PC5 NUMBER, PC6 NUMBER, PC7 NUMBER
);
...
-- 4) PC1 점수 상위 5명 (Oracle: FETCH FIRST, 12c+)
