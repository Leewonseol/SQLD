...
WITH grand AS (
    SELECT SUM(mean_score * n) / SUM(n) AS grand_mean, SUM(n) AS N, COUNT(*) AS k
    FROM branch_group_summary
),
ssb_ssw AS (
    SELECT
        SUM(b.n * (b.mean_score - g.grand_mean) * (b.mean_score - g.grand_mean)) AS SSB,
...
SELECT
    ...
    ROUND((SSB/(k-1)) / (SSW/(N-k)), 4) AS f_stat_from_sql
FROM ssb_ssw;
...
expected AS (
    SELECT c.variant, c.converted, c.n AS observed,
           1.0 * t.row_total * ct.col_total / g.grand_total AS expected
    FROM ab_contingency c
    JOIN totals t ON t.variant = c.variant
    JOIN col_totals ct ON ct.converted = c.converted
    CROSS JOIN grand g
)
SELECT ROUND(SUM((observed-expected)*(observed-expected)/expected), 4) AS chi2_stat_from_sql
FROM expected;
