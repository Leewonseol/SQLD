...
CREATE TABLE rf_feature_importance (feature VARCHAR2(30), importance NUMBER);
CREATE TABLE rf_metrics (n_estimators NUMBER, oob_score NUMBER, test_accuracy NUMBER);
CREATE TABLE rf_predictions (customer_id VARCHAR2(32), actual NUMBER, predicted NUMBER);
...
-- (08-의사결정나무/03_verify_oracle.sql이 먼저 실행되어 tree_feature_importance가 있어야 함)
SELECT 'decision_tree' AS model, feature, importance
