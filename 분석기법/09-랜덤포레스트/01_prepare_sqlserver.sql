-- 랜덤포레스트 입력 준비 (SQL Server)
-- 실행 상태: 실제 SQL Server 검증 필요
-- SQL Server에는 ROWID 개념 자체가 없다(Oracle 전용 의사컬럼) - 처음부터
-- ROW_NUMBER() OVER(...)로 순번을 만들어야 한다.

IF OBJECT_ID('rf_input', 'U') IS NOT NULL DROP TABLE rf_input;
SELECT
    f.customer_id,
    l.churned,
    f.satisfaction_score,
    f.days_since_last_order,
    f.order_count,
    f.membership_years,
    f.annual_income_10k,
    f.avg_order_value,
    f.age,
    CASE
        WHEN f.avg_order_value < 10 THEN '저액'
        WHEN f.avg_order_value < 20 THEN '중액'
        ELSE '고액'
    END AS aov_bucket,
    CASE WHEN ROW_NUMBER() OVER (ORDER BY f.customer_id) % 5 = 0 THEN 'test' ELSE 'train' END AS split
INTO rf_input
FROM customer_features f
JOIN customer_labels l ON l.customer_id = f.customer_id;

SELECT aov_bucket, COUNT(*) AS n, ROUND(AVG(1.0 * churned), 4) AS churn_rate
FROM rf_input
GROUP BY aov_bucket;
