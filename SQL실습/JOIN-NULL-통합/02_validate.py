"""JOIN×NULL 통합 실습 검증 스크립트.

../JOIN-표준JOIN/02_validate.py, ../NULL/02_validate.py와 완전히 동일한
방식으로 동작한다 - 모델을 학습하지 않고, 실제 DB 연결이 있으면 실행 결과를
비교하고, 없으면 정적 검증 모드로 전환한다. 자세한 원칙은
../_common/db_helpers.py 참고.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="JOIN×NULL 통합 실습 결과 검증")
