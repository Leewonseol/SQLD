...
-- score_times_2도 NULL. "0을 더해도 결과가 NULL"이라는 점이 핵심 —
-- NULL은 산술적으로 0이 아니라 "알 수 없음"이기 때문이다.

-- =====================================================================
-- STEP 2. NULL과 문자열 결합(||) — 하나라도 NULL이면 전체가 NULL이 된다.
--   (문자열 함수 심화는 05-NULL과문자열 폴더가 담당한다. 여기서는 "NULL
--   산술의 한 예시"로만 간단히 다룬다.)
-- =====================================================================
SELECT customer_id,
       nickname,
       nickname || '님' AS greeting_raw
FROM null_lab_customer
ORDER BY customer_id;
-- 예상(Oracle): nickname이 NULL인 2, 3(Oracle에서 ''→NULL), 8행은
-- greeting_raw도 NULL. Oracle에서는 '' 자체가 NULL이므로 3행도 함께
-- NULL이 된다는 점이 01 폴더 지식과 이어진다.

-- COALESCE/NVL로 방어 — NULL을 먼저 빈 문자열이나 대체 텍스트로 치환한
-- 뒤 결합하면 전체 결과가 NULL이 되는 것을 막을 수 있다.
SELECT customer_id,
       nickname,
       NVL(nickname, '고객') || '님' AS greeting_defended
FROM null_lab_customer
ORDER BY customer_id;
-- 예상: 모든 행에서 NULL 없이 문자열이 채워진다(2,3,8행 → '고객님').
...
-- *** 아래 SELECT는 실행하면 에러 발생 예상: ORA-01476: divisor is equal to zero ***
SELECT customer_id,
       order_qty,
       discount_qty,
...
-- "0으로 나누기=에러", "NULL로 나누기=NULL"이라는 서로 다른 현상이라는 것이
-- 이 STEP의 핵심.
...
-- NULLIF는 값 자체를 바꾸는 것이지 에러 여부의 원인을 없애는 것이 아니라,
-- "0을 NULL로 바꿔서 나눗셈이 애초에 0으로 나누는 상황 자체가 안 생기게"
-- 만드는 것이다.
